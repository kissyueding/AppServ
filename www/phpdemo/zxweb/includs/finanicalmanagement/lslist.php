...
$lsname = trim($_POST['lsname']);//接受用户传入数据
...
if($lsname==''){
    $row = "select * from liushui_info limit ".$start.",".$pageSize;
}
else{
    $row = "select * from liushui_info where lsname  LIKE '%$lsname%' limit ".$start.",".$pageSize;
}
...
$rowe=mysqli_query($conn,'select * from liushui_info');
